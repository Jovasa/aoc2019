...
crossings = {x: y for x, y in first_points.items() if x in second_points}

print(min(crossings, key=(lambda x: abs(x[0]) + abs(x[1]))))
temp = min(crossings, key=lambda x: first_points[x] + second_points[x])
print(first_points[temp] + second_points[temp])
